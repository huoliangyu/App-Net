...
class ClientFactory(ClientFactory, gobject.GObject):
    ...
    def connectionMade(self, host):
        ...
        localport = self.connector.transport.socket.getsockname()[1]
	print("~~~~timeout is "+str(self.connector.transport.socket.gettimeout()))
        setLocalport(localport);
        print("Detected localport "+str(localport))
        #tp.update_sock_addr(self.localport)
        #global socket_addr
        #socket_addr=self.localport
        #import code; code.interact(local=locals())
        #socket_addr=localport
        #set_socket_addr(localport)
        #print("my localport is "+str(self.localport))
            #import code; code.interact(local=locals())
