...
class TapasPlayer(object):
    ...
    def connectZMQ_pub(self):
	    ...
	    socket_send_app_param.bind("tcp://*:2222")
	...
	if self.first==1:
		 socket_send_app_param.send_string("%s %s %s %s %s %s" %("2","start",str(clientInfo),str(getLocalport()),str(os.getpid()),"1080p"))
	         self.first=0;
	
	else:
		socket_send_app_param.send_string("%s %s %s %s %s %s" %("2","client_update",str(clientInfo),str(getLocalport()),str(os.getpid()),"1080p"))
