...
class TapasPlayer(object):
    ...
    def fetchNextSegment(self): 
        ...
        '''
	print colored("FETCH THE NEXT SEGMENT IN LEVEL: "+str(self.getCurrentLevel()),'green')
	...
	print "-----------------------------------------------------------------------"
	print colored("my localport is "+str(getLocalport),'green')
	if not getLocalport() == 12:
		if self.first==1:
			 socket_send_app_param.send_string("%s %s %s %s %s %s" %("2","start",str(clientInfo),str(getLocalport()),str(os.getpid()),str(self.resolution)))
			 self.first=0;
		else:
			socket_send_app_param.send_string("%s %s %s %s %s %s" %("2","client_update",str(clientInfo),str(getLocalport()),str(os.getpid()),str(self.resolution)))
