...
class TapasPlayer(object):
    ...
    def startDownload(self, url, byterange=''):
	    ...
	    segSize=float(float(self.getLastFragmentBytes()*8))/1000;
            self.connection.makeRequest(path+"?buffer="+str(self.media_engine.getQueuedTime())+"?bwestim="+str(self.getBandwidth())+"?SegmentDur="+str(self.parser.getFragmentDuration())+"?SegmentSizeLa="+str(segSize)+"?quali="+str(self.getCurrentRate())+"?pid="+str(os.getpid()), byterange)
	...
	print colored("[playNextGotRequest()] I call fetchNextSegment() [1]",'red');
        self.fetchNextSegment()
	print colored("[playNextGotRequest()] I called fetchNextSegment() [2]",'red');
	...
	print colored("[_onConnectionMade] I call fetchNextSegment() [1]",'red');
        reactor.callLater(0.1, self.fetchNextSegment)
	print colored("[_onConnectionMade] I call fetchNextSegment() [2]",'red');
    ...
    def _onConnectionLost(self, connection):
	    ...
	    print colored("[_onConnectionLost] I call fetchNextSegment() [1]",'red');
            self.fetchNextSegment()
	    print colored("[_onConnectionLost] I call fetchNextSegment() [2]",'red');
    ...
    def _updatePlaylistDone(self, data):
	    ...
	    print colored("[updatePlaylistDone] I call fetchNextSegment() [1]",'red');
            self.fetchNextSegment()
	    print colored("[updatePlaylistDone] I called fetchNextSegment() [2]",'red');
        else:
	    print colored("[updatePlaylistDone] I call fetchNextSegment() [1]",'red');
