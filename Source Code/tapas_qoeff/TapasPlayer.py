...
from utils_py.connection import parse_url, ClientFactory, getLocalport
...
class TapasPlayer(object):
    ...
    def connectZMQ(self):
		...
		try:
		    ...
		    print colored("[3] execute fetchNExtSegment() from here",'blue')
		    print colored("[!!!!!!] level delegate i received: "+str(delegated_level))
	...
	define whether next segment will be prioritized or not
	'''
	print colored("updated the delegated Level to "+str(delegatedLevel),"yellow")
    ...
    def fetchNextSegment(self): 
        ...
        '''
	print ("FETCH THE NEXT SEGMENT IN LEVEL: "+str(self.getCurrentLevel()))
	...
	print colored ("[delegated level is ]"+str(self.delegatedLevel),'green')
	print "-----------------------------------------------------------------------"
	print colored ("my buffer is "+str(self.media_engine.getQueuedTime()),'red')
	print "-----------------------------------------------------------------------"
	if self.first==1:
		 socket_send_app_param.send_string("%s %s %s %s %s %s %s %s %s %s" %("2","start",str(self.getBandwidth()),str(segSize),str(clientInfo),str(getLocalport()),str(os.getpid()),str(self.resolution),'empt','empt'))
	         self.first=0;
	
	else:
		print("my idle time is "+str(idle_duration));
		print("my dl time is "+str(self.last_downloaded_time));
		socket_send_app_param.send_string("%s %s %s %s %s %s %s %s %s %s" %("2","client_update",str(self.getBandwidth()),str(segSize),str(clientInfo),str(getLocalport()),str(os.getpid()),str(self.resolution),str(idle_duration),str(self.last_downloaded_time)))
	...
	print colored("current buffer: "+str(self.media_engine.getQueuedTime()),'yellow')
	print colored('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~...Downloading...~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~','yellow')
